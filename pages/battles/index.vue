...
export default {
  ...
  methods: {
    handleSearch(query) {
      // Filtrar las batallas según el término de búsqueda
      this.filteredBattles = this.battles.filter((battle) => {
        const battleName = battle.name || ''; // Asegura que battleName sea una cadena vacía si es undefined
        return battleName.toLowerCase().includes(query.toLowerCase());
      });
    },
  },
};
